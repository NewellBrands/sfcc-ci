...
async function getFile(instance, path, file, token, ignoreLocalFilePath, options, callback) {
    // append file to post to request uri
    ...
    // build the request options
    var options = getOptions(instance, path, token, options, 'GET');

    // do the request, with request module
    request(options, callback).pipe(fs.createWriteStream(file));
    ...
    //       throw new Error(
    //         `Unexpected response status ${response.status} ${response.statusText} ${response.headers.get("Content-Type")} ${text}`
    //       )
    //     }
    //   } catch (err) {
    //     console.error("Error:", err) //failed to fetch
    //   } finally {
    //     console.debug(`< OmsClient:callToken return ${access_token}`)
    //     return access_token
    //   }
    // }
}
